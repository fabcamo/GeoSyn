...
from geoschemagen.create_schema import create_schema, create_schema_noRF, create_schema_eight_layers, \
    create_schema_eight_layers_noRF, create_schema_typeA, create_schema_typeB, create_schema_typeC, create_schema_typeD, create_schema_typeE, create_schema_typeF


def generate_database_OLD(output_folder: str,
                      no_realizations: int,
                      z_max: int,
                      x_max: int,
                      seed:int,
        ...
        try:
            ...
            if use_RF == True:
                if no_layers == 5:
                    ...
                    combine_trigo = 0
                    create_schema_typeB(output_folder, counter, z_max, x_max, combine_trigo, seed, True)
                else:
                ...
                if model_type == "A":
                    ...
                    create_schema_typeF(output_folder, counter, z_max, x_max, seed)
                else:
                    print("Number of layers not supported")
    ...
    while counter < no_realizations:
        try:
            ...
            if model_type == "A":
                # Just sine (1) or just cosine (2) per model
                combine_trigo = np.random.choice([1, 2])
                create_schema_typeA(output_folder, counter, z_max, x_max, combine_trigo, seed, use_RF)
            elif model_type == "B":
                # Mix of both sine and cosine in the same model
                combine_trigo = 0
